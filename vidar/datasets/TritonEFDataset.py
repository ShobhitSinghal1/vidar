...
class TritonEFDataset(BaseDataset):
    def __init__(self, tag=None, single_folder=False, split=None, extension='png', **kwargs):
        """
        Triton EF dataset, used to load information from folders
        ...
        self.tag = 'tritonEF' if tag is None else tag
    ...
    def __getitem__(self, idx):
        ...
        for _ in self.cameras:
            ...
            #     sample['rgb'][key] = im.crop((0, 0, width, 5 * height / 8))}
            ...
            if self.with_context:
                ...
                # img = read_image(filename_context)
                # s = {'rgb': {}}
                # for key, im in img.items():
                #     s['rgb'][key] = im.crop((0, 0, width, 5 * height / 8))
                # s = {0: img[0].crop((0, 0, width, 5 * height / 8))}
                # sample['rgb'].update(s)
